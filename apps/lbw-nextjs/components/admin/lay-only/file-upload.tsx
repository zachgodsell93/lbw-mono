...
export default function FileUpload({}: Props) {
  ...
  const processSelections = async (selections: Selection[]) => {
    ...
    const mp1Only = selections.filter((selection) => selection.MPRANK === 1);
    const mapSelections = mp1Only.map((selection: Selection) => {
      ...
      Papa.parse(file, {
        ...
        complete: async (result: any) => {
          ...
          const filteredData = sheetUpload.filter(
            (item: SheetUpload) => item.mpdiv2 && item.mpdiv2 <= 100
          );
  ...
  return (
    <div className="flex flex-col gap-y-4 w-full">
      <Card className="w-full">
        <CardHeader>
          <CardTitle>Upload MP1 & MP2 Selections</CardTitle>
          <CardDescription>
            Data required: DATE, HORSE, TN, RN, MP2DIV, MPRANK
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <TableRow key={index}>
              ...
              <TableCell>
                <Input
                  type="number"
                  className="w-30"
                  value={item.lbw_price?.toString() || ""}
                  onChange={(e) => {
                    const updatedData = uploadData.map((data, i) => {
                      if (i === index) {
                        return { ...data, lbw_price: Number(e.target.value) };
                      }
                      return data;
                    });
                    setUploadData(updatedData);
